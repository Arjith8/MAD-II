...
const album_id = ref(route.params.id)
...
onBeforeMount(async ()=>{
    ...
    const response = await fetch(`http://127.0.0.1:5000/api/v1/album?album_id=${album_id.value}`,{
        method:'GET',
        headers:{
    ...
    album_name.value = data.name
    genre.value = data.genre

    if (data.songs[0].song_name){
        song1.value = data.songs[0].song_name
    }
    if(data.songs[1].song_name){
        song2.value = data.songs[1].song_name
    }
    if(data.songs[2].song_name){
        song3.value = data.songs[2].song_name
    }
    if(data.songs[3].song_name){
        song4.value = data.songs[3].song_name
    }
    if(data.songs[4].song_name){
        song5.value = data.songs[4].song_name
    }
    if(data.songs[5].song_name){
        song6.value = data.songs[5].song_name
    }
    console.log(data)
})
...
async function createAlbum(){
    ...
    songs.value = []
    if (!album_name.value || !song1.value){
        alert("Please fill atleast the first song and the Album name")
        return
    }
    formData.append('album_id', album_id.value)
