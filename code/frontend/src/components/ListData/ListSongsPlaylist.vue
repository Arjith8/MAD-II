...
onBeforeMount(async()=>{
    ...
    const response = await fetch(`http://127.0.0.1:5000/api/v1/playlist?playlist_id=${playlist_id}`,{
        headers:{
            "Content-Type":"application/json",
            "Authorization":`${token.value}`
        }
    })
    playlist_data.value = await response.json()
    console.log(playlist_data.value)

    songs.value = playlist_data.value.songs
})
...
<template>
    ...
    <h1 class="text-white">{{ playlist_data.name }}</h1>
